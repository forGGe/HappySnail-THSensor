...
while True:
        ...
        mcu_temp = int(data.getRegister(2)) + int(data.getRegister(3)) / 100.0
        humidity = int(data.getRegister(4)) + int(data.getRegister(5)) / 100.0

        print('temperature: {}, humidity: {}, MCU temperature {}'.format(temperature, humidity, mcu_temp))

        telemetry = { 'temp': temperature, 'rh': humidity, 'mcu_temp': mcu_temp }
